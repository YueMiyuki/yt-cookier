...
module.exports = async function (browser, { email, pass, OTPtoken }) {
  try {
    ...
    if (!emailClick) throw new Error("Email box not found!");
    ...
    const passwordClick = await page.waitForSelector('input[type="password"]', {
      visible: true,
    });
    ...
    if (
      ...
      try {
        ...
        if (!totpClick) {
          throw new Error("A wrong password was provided!");
        } else {
          try {
        ...
        if (!Luri.startsWith("https://www.youtube.com/")) {
          console.log(
            "Wrong 2FA Secret is provided! Please check and try again."
          );
          await page.close();
          return "failed";
        } else if (
          ...
          console.log(
            "This account have no right to access youtube.com! Please try another account!"
          );
          await page.close();
          return "failed";
        } else if (
